...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!countdownTime || typeof countdownTime !== 'number' || countdownTime <= 0) {
      return NextResponse.json({ message: 'Invalid countdown time' }, { status: 400 });
    }

    const client = await clientPromise;
    const db = client.db('countdown');
    const countdownCollection = db.collection<{ _id: number; endTime: number }>('countdown');

    const existingCountdown = await countdownCollection.findOne({ _id: 1 });

    if (existingCountdown && existingCountdown.endTime > Date.now()) {
      const remainingTime = Math.floor((existingCountdown.endTime - Date.now()) / 1000);
      return NextResponse.json({ message: 'Timer already running', remainingTime }, { status: 200 });
    }
    ...
    return NextResponse.json({ message: 'Timer started', countdownEndTime }, { status: 200 });
  } catch (error) {
...
export async function GET() {
  try {
    ...
    if (!countdown || !countdown.endTime) {
      return NextResponse.json({ message: 'Timer not started yet', remainingTime: 0 }, { status: 400 });
    }
    ...
    if (remainingTime <= 0) {
      ...
      return NextResponse.json({ message: 'Countdown ended', remainingTime: 0 }, { status: 200 });
    }

    return NextResponse.json({ message: 'Countdown active', remainingTime: Math.floor(remainingTime / 1000) }, { status: 200 });
  } catch (error) {
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    if (!countdown || !countdown.endTime) {
      return NextResponse.json({ message: 'No active countdown to stop' }, { status: 400 });
    }

    await countdownCollection.deleteOne({ _id: 1 });

    return NextResponse.json({ message: 'Countdown stopped', remainingTime: 0 }, { status: 200 });
  } catch (error) {
